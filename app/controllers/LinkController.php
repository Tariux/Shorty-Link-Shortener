	...
	*/
    ...
    public function redirect(Request $request, string $alias){
		...
		if(!empty($url->expiry) && strtotime("now") > strtotime($url->expiry)) {
			if( isset($options['redirectexpiration']) && !empty($options['redirectexpiration'])){
				header('Location: '.$options['redirectexpiration']);
				exit;
			}else {
				return Gate::expired();
			}
		...
		if(!empty($url->options)){
			...
			if(isset($options['clicklimit']) && $options['clicklimit'] && $options['clicklimit'] > $url->click) {
				if( isset($options['redirectexpiration']) && !empty($options['redirectexpiration'])){
					header('Location: '.$options['redirectexpiration']);
					exit;
				}else {
