...
class Users {
    ...
    public function VerifyPhone(Request $request){
        ...
        if (!Auth::check()) {
            return Helper::redirect()->to("login")->with('danger', e('Please login.'));
        }
        $user = Auth::user();
        if ($user->phone_verify == true) {
            return \Core\Helper::redirect()->to(route('dashboard'));

        }


        View::set('title', e('Veify your phone number'));
        View::set("description", e('verify your phone number.'));
        ...
        try {
            $sender = "10008663";
            $receptor = $phone;
            $message = "تست احزار هویت پیامکی سامانه کوتاه کننده لینک" . $code;
            $api = new \Kavenegar\KavenegarApi("2F536C7A41556D5961754F475343365538734A6563463349673379307150726253673455614E76354C426F3D");
        ...
        return View::with('auth.verify_phone', compact('phone' , 'code'))->extend('layouts.auth');
    }

    public function VerifyPhoneAuth(Request $request){
    ...
    public function registerValidate(Request $request){
        ...
        if(!$request->email || !$request->username || !$request->password || !$request->phone) return Helper::redirect()->back()->with('danger', e('The email, the username , the phone and the password are required.'));

        $user = DB::user()->create();

        // jaryan update

        if(!$request->validate($request->phone, 'int')) return Helper::redirect()->back()->with('danger', e('Please enter a valid phone number.'));
        if(DB::user()->where('phone', $request->phone)->first()) return Helper::redirect()->back()->with('danger', e('Phone already exists.'));

        $user->phone = Helper::RequestClean($request->phone);
