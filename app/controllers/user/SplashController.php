...
class Splash {
    ...
    public function save(Request $request){
        ...
        if(!$image = $request->file('avatar')) return Helper::redirect()->back()->with('danger', e('Avatar must be either a PNG or a JPEG (Max 300kb).'));
			
        if(!$image->mimematch || !in_array($image->ext, ['jpeg', 'jpg', 'png'])) return Helper::redirect()->back()->with('danger', e('Avatar must be either a PNG or a JPEG (Max 300kb).'));
        ...
        if(!$image = $request->file('banner')) return Helper::redirect()->back()->with('danger', e('Banner must be either a PNG or a JPEG (Max 500kb).'));
			
        if(!$image->mimematch || !in_array($image->ext, ['jpeg', 'jpg', 'png'])) return Helper::redirect()->back()->with('danger', e('Banner must be either a PNG or a JPEG (Max 500kb).'));
